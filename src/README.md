...
React + TypeScript planner app. Game content lives in `data/` and is validated at load time — business logic should read from `GameData`, not hardcode LoreRim numbers.

## Layout

```
src/
  components/     # Reusable UI (perk trees, level bar, option details, etc.)
...
Skill levels, perk points, and skill points are **three separate systems**. See `data/README.md` and `.cursor/rules/giga-planner-data-model.mdc` before changing economy or perk fields.
...
CI runs `npm test` on every pull request (`.github/workflows/test.yml`).
